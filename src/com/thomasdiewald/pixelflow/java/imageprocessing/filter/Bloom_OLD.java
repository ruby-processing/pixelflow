...
public class Bloom_OLD {
  ...
  public Bloom_OLD(DwPixelFlow context){
...
//      tex_blur[i].resize(context, GLES3.GL_RGBA8, wi, hi, GLES3.GL_RGBA, GLES3.GL_UNSIGNED_BYTE, GLES3.GL_LINEAR, 4, 1);
      tex_temp[i].resize(context, GL3.GL_RGBA16F, wi, hi, GLES3.GL_RGBA, GLES3.GL_FLOAT, GLES3.GL_LINEAR, 4, 2);
      tex_blur[i].resize(context, GL3.GL_RGBA16F, wi, hi, GLES3.GL_RGBA, GLES3.GL_FLOAT, GLES3.GL_LINEAR, 4, 2);
...
//    for(int j = 0; j < 10; j++){
    ...
    for(int i = 1; i < BLUR_LAYERS; i++){
      filter.gaussblur.apply(tex_blur[i-1], tex_blur[i], tex_temp[i], 1 + i + radius);
    }
...
//    }
    ...
    if(dst_bloom != null){
      merge.apply(dst_bloom, tex_blur, tex_weights);
      if(dst_composition != null){
    ...
    context.pushRenderSettings(additive_blend);
    merge.apply(dst_composition, tex_blur, tex_weights);
    context.popRenderSettings();
